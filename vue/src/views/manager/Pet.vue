<template>
  ...
  <div>
    <div class="card" style="margin-bottom: 5px">
      <el-input v-model="data.name" style="width: 300px;margin-right: 10px" placeholder="请输入宠物名称搜索" prefix-icon="Search"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" stripe @selection-change="handleSelectionChange" tooltip-effect="dark myTooltip" v-if="data.user.role === 'ADM' || (data.user.role === 'PETSHOP' && data.form.sellPet === data.user.shopId)">
        ...
        <el-table-column prop="introduce" show-overflow-tooltip label="宠物介绍"></el-table-column>
        ...
        <el-table-column label="推荐" v-if="data.user.role === 'ADM'" width="200">
          <template v-slot="scope">
            <el-button plain type="success" size="default" @click="recommend(scope.row,'是')" :disabled="scope.row.recommend === '是'">推荐</el-button>
            <el-button plain type="danger" size="default" @click="recommend(scope.row,'否')" :disabled="scope.row.recommend === '否'">不推荐</el-button>
    ...
    <el-dialog title="宠物信息" v-model="data.formVisible" width="50%" destroy-on-close>
      <el-form ref="formRef" :rules="data.rules" :model="data.form" label-width="80px" style="margin-top: 20px;margin-right: 50px">
        ...
        <el-form-item prop="introduce" label="宠物详情">
        ...
        <el-form-item prop="recommend" label="是否推荐" v-if="data.user.role === 'ADM'">
          <el-radio-group v-model="data.form.recommend">
            <el-radio-button type="success" label="是">是</el-radio-button>
            <el-radio-button type="danger" label="否">否</el-radio-button>
          </el-radio-group>
        </el-form-item>
    ...
    <el-dialog title="宠物详情" v-model="data.viewVisible" width="50%" :close-on-click-modal="false">
...
request.get('/petType/selectAll').then(res => {
  ...
  // 解析用户出售宠物的信息，若未设置则默认为空数组
  let sellPet = JSON.parse(data.user.sellPet || '[]');

  // 过滤typeList，仅保留用户正在出售的宠物类型
...
const load = () => {
  request.get('/pet/selectPage', {
    ...
    data.tableData = res.data.list.map(item => {
      // 查找与当前项关联的商店名称
      const shop = data.shopList.find(shop => shop.id === item.shopId);
      // 查找与当前项关联的类型名称
      const type = data.typeList.find(type => type.id === item.typeId);
      // 返回新对象，包含原始项数据以及新的商店名称和类型名称字段
      return {
        ...item,
        shopName: shop ? shop.name : '',
        typeName: type ? type.name : '未知类型' // 修改：确保 typeName 不为空
      };
    });
...
const handleAdd = () => {
  ...
  data.form = { sex: '雄性', status: '上架', recommend: '否' ,shopId : data.user.id}
}
// 保存
...
const recommend = (row, recommend) => {
  let form = JSON.parse(JSON.stringify(row))  // 深拷贝一个对象，不会影响原对象
...
const del = (id) => {
  ElMessageBox.confirm('确认删除该员工信息吗？','删除确认', {type: 'warning'}).then(() => {
    request.delete('/pet/deleteById/' + id).then(res => {
      if (res.code === '200') {
...
const delBatch = () => {
  ...
  ElMessageBox.confirm('确认删除该员工信息吗？', '删除确认', {type: 'warning'}).then(() => {
    request.delete('/pet/deleteBatch', {data: data.ids}).then(res => {
      if (res.code === '200') {
        ElMessage.success('删除成功')
        load()
      } else {
